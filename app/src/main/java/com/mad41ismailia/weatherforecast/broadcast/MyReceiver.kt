package com.mad41ismailia.weatherforecast.broadcast
...
import com.mad41ismailia.weatherforecast.ALARM_ID
...
class MyReceiver : BroadcastReceiver() {
    ...
    override fun onReceive(context: Context, intent: Intent) {
        ...
        CoroutineScope(Dispatchers.Default).launch {
            ...
            if (alarm != null) {
                ...
                val maxDeg = currentData?.daily?.get(0)?.temp?.max?.toInt()
                val minDeg = currentData?.daily?.get(0)?.temp?.min?.toInt()
                val res = context.resources
                Log.i(
                    "alarmalarm",
                    "from receiver condition $condition value $value min $minDeg max $maxDeg"
                )
                if (condition.equals(res.getString(R.string.temp_more))) {
                    if (maxDeg != null && minDeg != null) {
    ...
    private fun setNewAlarm(id: String?) {
        ...
        val alarmManager =
            myContext.applicationContext.getSystemService(AppCompatActivity.ALARM_SERVICE) as AlarmManager
    ...
    private fun startNotification(condition: String, value: Int) {
        val builder: NotificationCompat.Builder =
            NotificationCompat.Builder(myContext, "ALARM_CHANNEL")
        ...
        val notificationManagerCompat: NotificationManagerCompat =
            NotificationManagerCompat.from(myContext)
